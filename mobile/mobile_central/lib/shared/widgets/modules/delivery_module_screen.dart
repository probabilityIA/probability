...
/// Module wrapper that groups Routes, Drivers and Vehicles
/// behind a TabBar, replicating the Next.js "subnavbar" pattern.
class DeliveryModuleScreen extends StatefulWidget {
  final int initialTab;

  const DeliveryModuleScreen({super.key, this.initialTab = 0});

  @override
  State<DeliveryModuleScreen> createState() => _DeliveryModuleScreenState();
}

class _DeliveryModuleScreenState extends State<DeliveryModuleScreen> {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            RouteListScreen(
              key: ValueKey('routes_$effectiveBusinessId'),
              businessId: effectiveBusinessId,
            ),
            DriverListScreen(
              key: ValueKey('drivers_$effectiveBusinessId'),
              businessId: effectiveBusinessId,
            ),
            VehicleListScreen(
              key: ValueKey('vehicles_$effectiveBusinessId'),
              businessId: effectiveBusinessId,
            ),
          ],
        ),
  ...
  PreferredSizeWidget _buildTabBar() {
    return const TabBar(
      ...
      tabs: [
        ...
        Tab(icon: Icon(Icons.badge), text: 'Conductores'),
