...
Rules in `.claude/rules/infra-ops.md` override any generic advice here.

1. **Every command needs `--profile probability --region us-east-1`.**
2. **Cost Explorer API calls cost $0.01 each** (`GetCostAndUsage`,
   `GetCostForecast`). Do not loop over months or services without saying so
   first. Cache and reuse results within a session.
3. **Do not enable CUR + Athena** without asking. It adds S3 storage plus a
   per-query Athena charge, and this account's whole monitoring setup is
   deliberately inside the free tier. `references/cur-athena.md` is reference
   material, not a recommendation.
4. **Budgets: 2 are free**, none created yet - this is a known pending item.
   Point notifications at the existing SNS topic
   `arn:aws:sns:us-east-1:476702565908:probability-alertas` or at
   probabilitysas@gmail.com, which already receives Cost Anomaly Detection.
5. **Cost Anomaly Detection is already active** - do not create a duplicate.
6. Right-sizing context: prod is a single `t4g.small` EC2 running 8 containers
   plus an RDS PostgreSQL 15. RAM is the tight resource, not CPU. Compute
   Optimizer advice to downsize should be treated with suspicion; the machine
   is already small and Prometheus/Grafana were left off for that reason.

Account: `476702565908`.
...
| Deep-dive on a specific EC2/Lambda/EBS/RDS rec? | Compute Optimizer | `references/ec2-rightsizing.md`, `references/rds-optimization.md`, `references/ebs-optimization.md` |
