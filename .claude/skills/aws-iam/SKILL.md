...
1. **Every command needs `--profile probability --region us-east-1`.**
   Account `476702565908`. Never operate as root.
2. **Ask before any IAM mutation.** IAM changes here are outward-facing and
   hard to reverse. Propose the policy JSON and wait for approval.
3. **The restrictions in the existing policies are deliberate, not oversights.**
   Do not "fix" them by widening scope:
   - `ProbabilityRdsTunnelSSM` - only opens the SSM port-forward. No RDS or EC2
     permissions ON PURPOSE.
   - `ProbabilityOpsSantiago` - `ssm:StartSession` / `ssm:SendCommand` are
     restricted to instance `i-0f3284d2a87127e57`. It deliberately CANNOT
     create or delete EC2/RDS, touch security groups, or administer other
     people's IAM.
4. **Never propose reopening a port to `0.0.0.0/0`** or setting
   `PubliclyAccessible=true` on the RDS. Only 80/443 are public. If something
   cannot connect, the answer is the SSM tunnel, not an ingress rule.
5. Users: `terraform-github-admin` (infra/CI), `santiago.camacho` (ops),
   `backend-s3-uploader` (app S3), `docker-ecr-dev` (ECR push), `bedrik-IA` and
   `woo-store-power` (integrations).
6. Credentials go through a private channel - never chat, email, or git.
