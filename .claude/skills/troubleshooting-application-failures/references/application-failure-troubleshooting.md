...
- You MUST run every `aws` command through the Bash tool. There is no `call_aws`
  tool in this project and none is needed.
- You MUST append `--profile probability --region us-east-1` to every command.
- You MUST NOT run mutating commands. This procedure is read-only.
...
- You MUST search for log groups that contain the application name using: `aws logs describe-log-groups --profile probability --region us-east-1`
...
- You MUST validate each confirmed log group using: `aws logs describe-log-groups --log-group-name-prefix ${log_group_name} --profile probability --region us-east-1`
- You MUST list available log streams for each log group: `aws logs describe-log-streams --log-group-name ${log_group_name} --order-by LastEventTime --descending --max-items 10 --profile probability --region us-east-1`
...
- You MUST start queries to search for errors and failure patterns:
  - **Error Query**: `aws logs start-query --log-group-name ${log_group_name} --start-time ${start_timestamp} --end-time ${end_timestamp} --query-string 'fields @timestamp, @message | filter @message like /(?i)(error|fail|exception|timeout|unable|denied|invalid)/ | sort @timestamp desc | limit 100' --profile probability --region us-east-1`
  - **Exception Query**: `aws logs start-query --log-group-name ${log_group_name} --start-time ${start_timestamp} --end-time ${end_timestamp} --query-string 'fields @timestamp, @message | filter @message like /(?i)(exception|stack trace|caused by|at .+\\.java:|at .+\\.py:)/ | sort @timestamp desc | limit 100' --profile probability --region us-east-1`
...
- You MUST poll each query status using: `aws logs get-query-results --query-id ${query_id} --profile probability --region us-east-1`
