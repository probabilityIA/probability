...
export default async function ConfiguracionPage() {
    const [conceptsResult, taxesResult, servicesResult, dianResult] = await Promise.all([
        getAccountingConceptsAction(),
        getAccountingTaxesAction(),
        getAccountingServicesAction(),
        getAccountingDianConfigAction(),
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 w-full px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
            <AccountingGate>
                <ConfigView concepts={concepts} taxes={taxes} services={services} dianConfig={dianConfig} error={error} />
            </AccountingGate>
        </div>
    );
}
