...
import {
    Search, RefreshCw, ChevronLeft, ChevronRight, Package, AlertCircle, CheckCircle2, Clock, Lock,
} from 'lucide-react';
...
export default function CodOrdersTab({ filters }: Props) {
    ...
    useEffect(() => { setPage(1); }, [filters, collected, debounced]);
    ...
    const load = useCallback(async () => {
        ...
        setLoading(false);
    }, [filters, page, collected, debounced]);
                        ...
                        {!loading && orders.map(o => {
                            ...
                            return (
                                <tr key={o.order_id} className="border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                    ...
                                    <td className="px-3 py-2 text-right text-gray-500">{o.discount_pct > 0 ? `${o.discount_pct.toFixed(1)}%` : '-'}</td>
                                    <td className="px-3 py-2 text-right font-semibold text-purple-700 dark:text-purple-300">{formatMoney(o.net, o.currency)}</td>
