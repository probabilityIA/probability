...
import { useSiigoReferrals } from '../hooks/useSiigoReferrals';

const ORDER_RANGE_LABELS: Record<string, string> = {
    '0-50': '0 a 50 órdenes/mes',
    '51-200': '51 a 200 órdenes/mes',
    '201-500': '201 a 500 órdenes/mes',
    '501-1000': '501 a 1000 órdenes/mes',
    '1000+': 'Más de 1000 órdenes/mes',
};

export const SiigoReferralsList: React.FC = () => {
    const {
        referrals,
        loading,
        error,
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Referidos Siigo</h1>
            </div>
                    ...
                    <table className="table min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead style={{ backgroundColor: 'var(--color-primary)', color: 'white' }}>
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Órdenes/mes</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={4} className="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
                                        No hay referidos capturados todavía
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={referral.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                            ...
                                            <div className="text-xs font-normal text-gray-400">
                                                {new Date(referral.created_at).toLocaleString('es-CO')}
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            {ORDER_RANGE_LABELS[referral.order_range] || referral.order_range}
                                        </td>
                                    </tr>
                                ))
                            )}
