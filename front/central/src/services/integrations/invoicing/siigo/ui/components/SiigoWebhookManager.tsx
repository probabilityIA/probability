...
import type { SiigoWebhookInfo } from '../../domain/types';

interface SiigoWebhookManagerProps {
...
function shortTopic(topic: string): string {
    return topic.replace('public.siigoapi.', '');
}

export function SiigoWebhookManager({ integrationId }: SiigoWebhookManagerProps) {
    const { showToast } = useToast();
    const [webhooks, setWebhooks] = useState<SiigoWebhookInfo[]>([]);
    ...
    const handleCreate = async () => {
        ...
        if (result.success === false) {
            ...
            showToast(result.message || 'Webhooks de inventario creados', 'success');
            await fetchWebhooks();
        }
        setCreating(false);
    };
    ...
    const handleDelete = async (webhookId: string, topic: string) => {
        if (!confirm(`Eliminar webhook "${shortTopic(topic)}"?`)) return;
        ...
        <div className="space-y-3">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold text-gray-900 dark:text-white">Webhooks de inventario</h3>
                    {webhooks.length > 0 && (
                        <span
            ...
            <p className="text-[11px] text-gray-400 dark:text-gray-500">
                Siigo notificara a Probability cuando cambie el stock o se creen/actualicen productos, para sincronizar el inventario automaticamente.
            </p>

            {webhooks.length === 0 ? (
                <p className="text-[11px] text-gray-400 dark:text-gray-500">
                    No hay webhooks configurados. Crea los webhooks para recibir eventos de inventario de Siigo.
                </p>
            ) : (
                <div
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {webhooks.map((wh) => (
                                <tr key={wh.id} className="hover:bg-gray-50/60 dark:hover:bg-gray-700/40">
                                    <td className="px-3 py-2 font-mono text-[11px] text-gray-800 dark:text-gray-200 whitespace-nowrap">{shortTopic(wh.topic)}</td>
                                    <td className="px-3 py-2 text-[11px] text-gray-500 dark:text-gray-400 max-w-[260px] truncate" title={wh.url}>
                                        {wh.url}
                                    </td>
                                    <td className="px-3 py-2">
                                        {wh.active ? (
                                            <span className="inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800">
                                                Activo
                                            </span>
                                        ) : (
                                            <span className="inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase bg-gray-100 text-gray-600 border border-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600">
                                                Inactivo
                                            </span>
                                        )}
                                    </td>
