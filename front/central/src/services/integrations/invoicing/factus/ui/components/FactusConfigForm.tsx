...
interface FactusConfigFormProps {
    onSuccess?: () => void;
    onCancel?: () => void;
}

export function FactusConfigForm({ onSuccess, onCancel }: FactusConfigFormProps) {
    ...
    const [formData, setFormData] = useState({
        name: '',
        numbering_range_id: '' as string | number,
    ...
    const handleTestConnection = async () => {
        if (!formData.client_id || !formData.client_secret || !formData.username || !formData.password) {
            showToast('Debes ingresar Client ID, Client Secret, Usuario y Contraseña para probar la conexión', 'warning');
            return;
        }
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const response = await createIntegrationAction({
                name: formData.name,
                code: `factus_${Date.now()}`,
                integration_type_id: 7, // Factus integration type ID (from database)
                category: 'invoicing',
            ...
            if (response.success) {
                showToast('Integración Factus creada exitosamente', 'success');
                onSuccess?.();
            } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 space-y-4 border border-green-100">
                ...
                <p className="text-sm text-green-900 -mt-2 mb-4 flex items-start gap-2">
                    ...
                    <span>
                        Obtén tus credenciales desde el panel de Factus en la sección
                ...
                <div className="bg-green-100 border border-green-200 rounded-lg p-4 mt-4">
                    ...
                    <ol className="text-xs text-green-800 space-y-2 list-decimal list-inside ml-1">
                        <li>Ingresa a tu cuenta en <strong>app.factus.com.co</strong></li>
                        <li>Ve a <strong>Configuraci&oacute;n → API → Credenciales</strong></li>
                        <li>Copia el <strong>Client ID</strong> y <strong>Client Secret</strong></li>
                        <li>El <strong>Usuario</strong> y <strong>Contraseña</strong> son los mismos que usas para iniciar sesi&oacute;n en Factus</li>
                    </ol>
                </div>
            </div>
            ...
            <div className="bg-blue-50 rounded-xl p-6 space-y-4 border border-blue-100">
                ...
                <div>
                    ...
                    <Input
                        ...
                        onChange={(e) => setFormData({ ...formData, numbering_range_id: e.target.value })}
                        placeholder="Ej: 1"
                        min="1"
                        required
                        className="bg-white"
