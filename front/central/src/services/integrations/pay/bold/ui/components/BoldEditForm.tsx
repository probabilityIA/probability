        ...
        <div className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600">
            ...
            <div className="flex-1">
                <div className="flex items-center gap-2">
                    {icon}
                    <span className={`font-semibold ${titleClass}`}>{checked ? titleOn : titleOff}</span>
                </div>
                {description && <p className="text-sm text-gray-600 dark:text-gray-300 mt-0.5">{description}</p>}
            </div>
        </div>
    );
}
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const response = await updateIntegrationAction(integrationId, {
                ...
                config: { ...initialData.config, use_platform_token: true },
                is_active: isActive,
                is_testing: isTesting,
            } as any);

            if (response.success) {
                showToast('Integración Bold actualizada', 'success');
                onSuccess?.();
            } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-6" autoComplete="off">
            <div className="border-b border-gray-200 dark:border-gray-700 pb-6">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Editar Bold</h2>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-300 ml-14">
                    Activa, desactiva o cambia el modo sandbox de Bold para este negocio.
                </p>
            </div>
