        ...
        <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
            <div className="border-b border-gray-200 dark:border-gray-700 pb-6">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Editar ePayco Pagos</h2>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-300 ml-14">Actualiza la configuración de tu integración con ePayco.</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-xl p-6 space-y-4 border border-teal-100">
                ...
                <div>
                    ...
                    <Select value={formData.environment} onChange={(e) => setFormData({ ...formData, environment: e.target.value as 'test' | 'production' })} options={[{ value: 'test', label: 'Test (Pruebas)' }, { value: 'production', label: 'Producción' }]} className="bg-white dark:bg-gray-800" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 dark:text-gray-200 mb-2">Customer ID</label>
                    <Input type="text" value={formData.customer_id} onChange={(e) => setFormData({ ...formData, customer_id: e.target.value })} placeholder="Dejar vacío para mantener el actual" autoComplete="off" data-1p-ignore className="bg-white dark:bg-gray-800 font-mono text-sm" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 dark:text-gray-200 mb-2">Key (P_KEY)</label>
                    <div className="relative">
                        <Input type={showKey ? 'text' : 'password'} value={formData.key} onChange={(e) => setFormData({ ...formData, key: e.target.value })} placeholder="Dejar vacío para mantener la actual" autoComplete="new-password" data-1p-ignore className="bg-white dark:bg-gray-800 font-mono text-sm pr-10" />
                        <button type="button" onClick={() => setShowKey(!showKey)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 dark:text-gray-400 hover:text-gray-700 dark:text-gray-200 dark:text-gray-200 focus:outline-none" tabIndex={-1}>
