...
interface NequiEditFormProps {
    integrationId: number;
    initialData: {
        name: string;
        config: NequiConfig;
        credentials?: NequiCredentials;
...
export function NequiEditForm({ integrationId, initialData, onSuccess, onCancel }: NequiEditFormProps) {
    ...
    const [formData, setFormData] = useState({
        ...
        phone_code: initialData.config.phone_code || '+57',
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (response.success) {
                showToast('Integración Nequi actualizada exitosamente', 'success');
                onSuccess?.();
            } else {
            ...
            showToast('Error al actualizar integración Nequi', 'error');
        } finally {
            setLoading(false);
        }
    };
