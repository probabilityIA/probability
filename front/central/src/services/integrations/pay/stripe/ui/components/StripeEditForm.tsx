    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
            <div className="border-b border-gray-200 pb-6">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300 ml-14">Actualiza la configuración de tu integración con Stripe.</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-violet-50 rounded-xl p-6 space-y-4 border border-indigo-100">
                ...
                <div>
                    ...
                    <Select value={formData.environment} onChange={(e) => setFormData({ ...formData, environment: e.target.value as 'test' | 'live' })} options={[{ value: 'test', label: 'Test (Pruebas)' }, { value: 'live', label: 'Live (Producción)' }]} className="bg-white" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Secret Key</label>
                    <div className="relative">
                        <Input type={showSecretKey ? 'text' : 'password'} value={formData.secret_key} onChange={(e) => setFormData({ ...formData, secret_key: e.target.value })} placeholder="Dejar vacío para mantener la actual" autoComplete="new-password" data-1p-ignore className="bg-white font-mono text-sm pr-10" />
                        <button type="button" onClick={() => setShowSecretKey(!showSecretKey)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-200 focus:outline-none" tabIndex={-1}>
            ...
            <div className="flex justify-between items-center gap-3 pt-6 border-t border-gray-200">
                {onCancel && (
                    <Button type="button" onClick={onCancel} disabled={loading} className="min-w-[140px] bg-gray-100 hover:bg-gray-200 text-gray-700 dark:text-gray-200 border border-gray-300">
