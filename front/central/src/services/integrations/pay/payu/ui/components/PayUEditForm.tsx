        ...
        <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
            <div className="border-b border-gray-200 dark:border-gray-700 pb-6">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Editar PayU Pagos</h2>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-300 ml-14">Actualiza la configuración de tu integración con PayU.</p>
            </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 space-y-4">
                ...
                <div>
                    ...
                    <Input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required className="bg-white dark:bg-gray-800" />
                </div>
                {isSuperAdmin && (
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 dark:text-gray-200 mb-2">Negocio</label>
                        {loadingBusinesses ? (
                            ...
                            <Select value={selectedBusinessId?.toString() || ''} onChange={(e) => setSelectedBusinessId(Number(e.target.value))} options={[{ value: '', label: '-- Selecciona un negocio --' }, ...businesses.map((b) => ({ value: b.id.toString(), label: b.name }))]} disabled={true} className="bg-white dark:bg-gray-800" />
                        )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 dark:text-gray-200 mb-2">Account ID</label>
                        <Input type="text" value={formData.account_id} onChange={(e) => setFormData({ ...formData, account_id: e.target.value })} placeholder="ID de tu cuenta PayU" className="bg-white dark:bg-gray-800" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 dark:text-gray-200 mb-2">Merchant ID</label>
                        <Input type="text" value={formData.merchant_id} onChange={(e) => setFormData({ ...formData, merchant_id: e.target.value })} placeholder="ID de tu comercio PayU" className="bg-white dark:bg-gray-800" />
                    </div>
                </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-6 space-y-4 border border-orange-100">
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 dark:text-gray-200 mb-2">Ambiente</label>
                    <Select value={formData.environment} onChange={(e) => setFormData({ ...formData, environment: e.target.value as 'sandbox' | 'production' })} options={[{ value: 'sandbox', label: 'Sandbox (Pruebas)' }, { value: 'production', label: 'Producción' }]} className="bg-white dark:bg-gray-800" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 dark:text-gray-200 mb-2">API Login</label>
                    <Input type="text" value={formData.api_login} onChange={(e) => setFormData({ ...formData, api_login: e.target.value })} placeholder="Dejar vacío para mantener el actual" autoComplete="off" data-1p-ignore className="bg-white dark:bg-gray-800 font-mono text-sm" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 dark:text-gray-200 mb-2">API Key</label>
                    <div className="relative">
                        <Input type={showApiKey ? 'text' : 'password'} value={formData.api_key} onChange={(e) => setFormData({ ...formData, api_key: e.target.value })} placeholder="Dejar vacío para mantener la actual" autoComplete="new-password" data-1p-ignore className="bg-white dark:bg-gray-800 font-mono text-sm pr-10" />
                        <button type="button" onClick={() => setShowApiKey(!showApiKey)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 dark:text-gray-400 hover:text-gray-700 dark:text-gray-200 dark:text-gray-200 focus:outline-none" tabIndex={-1}>
            ...
            <div className="flex justify-between items-center gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                {onCancel && (
                    <Button type="button" onClick={onCancel} disabled={loading} className="min-w-[140px] bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 text-gray-700 dark:text-gray-200 dark:text-gray-200 border border-gray-300 dark:border-gray-600">
