...
interface EnvioClickEditFormProps {
...
export function EnvioClickEditForm({ integrationId, initialData, onSuccess, onCancel }: EnvioClickEditFormProps) {
    const { showToast } = useToast();
    const [loading, setLoading] = useState(false);
    const [testingConnection, setTestingConnection] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [showApiKey, setShowApiKey] = useState(false);
    const [usePlatformToken, setUsePlatformToken] = useState(initialData.config?.use_platform_token || false);
    ...
    const handleTestConnection = async () => {
        ...
        try {
            ...
            const result = await testConnectionRawAction('envioclick', { use_platform_token: usePlatformToken }, credentials);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            // Only include credentials if using own API key and a key was entered
            if (!usePlatformToken && formData.api_key) {
                updateData.credentials = { api_key: formData.api_key } as EnvioClickCredentials;
            }

            const response = await updateIntegrationAction(integrationId, updateData);

            if (response.success) {
                showToast('Integracion EnvioClick actualizada exitosamente', 'success');
                onSuccess?.();
            } else {
            ...
            showToast('Error al actualizar integracion EnvioClick', 'error');
        } finally {
            setLoading(false);
        }
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
            ...
            <div className="bg-orange-50 rounded-xl p-6 space-y-4 border border-orange-200">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900">Modo de Pruebas</h3>
                </div>

                <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-orange-200">
                    <div>
                        <p className="text-sm font-medium text-gray-900">Activar modo de pruebas</p>
                        <p className="text-xs text-gray-500 mt-0.5">
                    ...
                    <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                            isTesting ? 'bg-orange-500' : 'bg-gray-300'
                        }`}
                        role="switch"
                ...
                {isTesting && (
                    <Alert type="warning">
                        Modo de pruebas activado. Los envios generados con esta integracion quedaran marcados como <strong>TEST</strong> en el sistema.
