...
import { TuConfig, TuCredentials } from '../../domain/types';
...
interface TuConfigFormProps {
    onSuccess?: () => void;
    onCancel?: () => void;
}

export function TuConfigForm({ onSuccess, onCancel }: TuConfigFormProps) {
    ...
    const handleTestConnection = async () => {
        ...
        try {
            const config: TuConfig = {
                base_url: formData.base_url || undefined,
            };

            const credentials: TuCredentials = {
                api_key: formData.api_key,
            };

            const result = await testConnectionRawAction('tu', config, credentials);

            if (result.success) {
                showToast('Conexion exitosa con TU', 'success');
            } else {
                throw new Error(result.message || 'Error al probar conexion');
            }
        } catch (err: any) {
            setError(err.message || 'Error al probar conexion');
            showToast('Error al conectar con TU: ' + err.message, 'error');
        } finally {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (response.success) {
                showToast('Integracion TU creada exitosamente', 'success');
                onSuccess?.();
            } else {
            ...
            showToast('Error al crear integracion TU', 'error');
        } finally {
            setLoading(false);
        }
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
            ...
            <div className="border-b border-gray-200 pb-6">
                ...
                <p className="text-sm text-gray-600 ml-14">
                    Conecta tu cuenta de TU para gestionar envios y cotizaciones de paqueteria desde Probability.
                </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-6 space-y-4 border border-orange-100">
                ...
                <p className="text-sm text-orange-900 -mt-2 mb-4 flex items-start gap-2">
                    ...
                    <span>
                        Obtiene tu API Key desde el panel de TU en la seccion de
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <Input
                                ...
                                placeholder="Ingresa tu API Key de TU"
                                required
                                autoComplete="off"
                                data-1p-ignore
            ...
            <div className="bg-gray-50 rounded-xl p-6 space-y-4">
                ...
                <div>
                    ...
                    <Input
                        ...
                        placeholder="https://api.tu.com/api/v1"
                        className="bg-white font-mono text-sm"
                    />
                    <p className="text-xs text-gray-500 mt-1.5">
                        URL base del servicio de TU (no modificar salvo indicacion del soporte)
                    </p>
                </div>
            </div>
