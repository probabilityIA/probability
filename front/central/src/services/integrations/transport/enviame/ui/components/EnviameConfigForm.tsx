...
interface EnviameConfigFormProps {
    onSuccess?: () => void;
    onCancel?: () => void;
}

export function EnviameConfigForm({ onSuccess, onCancel }: EnviameConfigFormProps) {
    ...
    const handleTestConnection = async () => {
        ...
        try {
            const config: EnviameConfig = {
                base_url: formData.base_url || undefined,
            };

            const credentials: EnviameCredentials = {
                api_key: formData.api_key,
            };

            const result = await testConnectionRawAction('enviame', config, credentials);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const response = await createIntegrationAction({
                name: formData.name,
                code: `enviame_${Date.now()}`,
                integration_type_id: 13,
                category: 'shipping',
            ...
            if (response.success) {
                showToast('Integracion Enviame creada exitosamente', 'success');
                onSuccess?.();
            } else {
            ...
            showToast('Error al crear integracion Enviame', 'error');
        } finally {
            setLoading(false);
        }
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
            ...
            <div className="border-b border-gray-200 pb-6">
                ...
                <p className="text-sm text-gray-600 ml-14">
                    Conecta tu cuenta de Enviame para gestionar envios y cotizaciones de paqueteria desde Probability.
                </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 space-y-4 border border-blue-100">
                ...
                <p className="text-sm text-blue-900 -mt-2 mb-4 flex items-start gap-2">
                    ...
                    <span>
                        Obtiene tu API Key desde el panel de Enviame en la seccion de
            ...
            <div className="bg-gray-50 rounded-xl p-6 space-y-4">
                ...
                <div>
                    ...
                    <Input
                        ...
                        placeholder="https://api.enviame.io/api/v2"
                        className="bg-white font-mono text-sm"
                    />
                    <p className="text-xs text-gray-500 mt-1.5">
                        URL base del servicio de Enviame (no modificar salvo indicacion del soporte)
                    </p>
                </div>
            </div>
