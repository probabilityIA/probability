...
import { syncMeliInventoryAction } from '../../infra/actions';

interface MercadoLibreInventorySyncModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const INVENTORY_EVENT_TYPES = ['meli.inventory.sync.started', 'meli.inventory.sync.progress', 'meli.inventory.sync.completed'];
...
export function MercadoLibreInventorySyncModal({ isOpen, onClose, integrationId, businessId, onCompleted }: MercadoLibreInventorySyncModalProps) {
    ...
    const handleMessage = useCallback((event: MessageEvent) => {
        try {
            ...
            if (eventType === 'meli.inventory.sync.started') {
                setTotal(Number(data.total) || 0);
                setPhase('running');
            } else if (eventType === 'meli.inventory.sync.progress') {
            ...
            } else if (eventType === 'meli.inventory.sync.completed') {
