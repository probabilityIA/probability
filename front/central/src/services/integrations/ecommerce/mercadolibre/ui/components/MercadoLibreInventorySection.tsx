...
export interface MeliInventoryConfig {
    enabled: boolean;
    mode: 'single' | 'sum';
...
interface MercadoLibreInventorySectionProps {
    value: MeliInventoryConfig;
    onChange: (v: MeliInventoryConfig) => void;
...
export function MercadoLibreInventorySection({ value, onChange, businessId, integrationId, onSyncNow, canSyncNow }: MercadoLibreInventorySectionProps) {
    ...
    const set = (patch: Partial<MeliInventoryConfig>) => onChange({ ...value, ...patch });
        ...
        >
            <div className="flex items-center justify-between gap-3">
                <div className="flex items-start gap-2">
                    <CubeIcon className="w-4 h-4 mt-0.5" style={{ color: GREEN }} />
                    <div>
                        <h4 className="text-[13px] font-bold text-gray-900 dark:text-gray-100">Sincronizacion de inventario</h4>
                        <p className="mt-0.5 text-[11px] text-gray-400 dark:text-gray-500">
                            Empuja el stock de Probability a tus publicaciones de MercadoLibre (available_quantity).
                        </p>
                    </div>
                </div>
                <button
                    ...
                    style={value.enabled ? { backgroundColor: GREEN } : undefined}
                >
            ...
            {value.enabled && (
                <div className="mt-4 space-y-4">
                    <div>
                        <label className={fieldLabel}>Que stock enviar a MercadoLibre</label>
                        <div className="grid grid-cols-2 gap-2">
                            <button
                                ...
                                style={value.mode === 'single'
                                    ? { borderColor: GREEN, backgroundColor: GREEN_SOFT }
