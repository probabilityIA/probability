...
const PRODUCT_EVENT_TYPES = [
    'jumpseller.product.reconcile.started',
    'jumpseller.product.reconcile.completed',
...
export function JumpsellerProductSyncModal({ isOpen, onClose, integrationId, businessId, onCompleted }: JumpsellerProductSyncModalProps) {
    ...
    const aplicarConteos = useCallback(async (data: any) => {
        ...
        setDiff({
            ...
            onlyInJumpseller: soloCanal,
            probabilityNoSku: Number(data.probability_no_sku) || 0,
            jumpsellerNoSku: Number(data.jumpseller_no_sku) || 0,
        });
        setPhase('diff');
    }, [cargarGrupo]);
    ...
    const handleMessage = useCallback((event: MessageEvent) => {
        try {
            ...
            if (!data) return;
            if (eventType === 'jumpseller.product.reconcile.started') return;

            if (eventType === 'jumpseller.product.reconcile.completed') {
                                ...
                                <>
                                    {diff.onlyInProbability.length > 0 && (
                                        <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                                            <div className="flex items-start justify-between gap-3">
                                                <div>
                                                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">En Probability hay {diff.onlyInProbability.length} producto{diff.onlyInProbability.length !== 1 ? 's' : ''} que no {'est\u00e1n'} en Jumpseller</p>
                                    ...
                                    {diff.onlyInJumpseller.length > 0 && (
                                        <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                                            <div className="flex items-start justify-between gap-3">
                                                <div>
                                                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">En Jumpseller hay {diff.onlyInJumpseller.length} producto{diff.onlyInJumpseller.length !== 1 ? 's' : ''} que no {'est\u00e1n'} en Probability</p>
