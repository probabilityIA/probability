...
import { ExitoCredentials } from '../../domain/types';
...
export function ExitoEditForm({ integrationId, initialData, onSuccess, onCancel }: ExitoEditFormProps) {
    ...
    const handleTestConnection = async () => {
        if (!formData.seller_id || !formData.api_key) {
            showToast('Debes ingresar Seller ID y API Key para probar la conexion', 'warning');
            return;
        }
        ...
        try {
            ...
            const result = await testConnectionRawAction('exito', {}, credentials);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (formData.seller_id && formData.api_key) {
                const credentials: ExitoCredentials = {
                    seller_id: formData.seller_id,
                    api_key: formData.api_key,
                };
        ...
        <form onSubmit={handleSubmit} className="space-y-8" autoComplete="off">
            ...
            <div className="border-b border-gray-200 dark:border-gray-700 pb-6">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                        Editar Exito Marketplace
                    </h2>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-300 ml-14">
                    Actualiza la configuracion de tu integracion con Exito.
                </p>
            </div>
            ...
            <div className="bg-gradient-to-br from-yellow-50 to-amber-50 rounded-xl p-6 space-y-4 border border-yellow-100">
                ...
                <div className="pt-2">
                    <Button
                        type="button"
                        variant="outline"
                        className="w-full bg-white dark:bg-gray-800 hover:bg-yellow-50 border-yellow-200 text-yellow-700 font-semibold"
                        onClick={handleTestConnection}
                        disabled={testingConnection || loading || !formData.seller_id || !formData.api_key}
                    >
                        {testingConnection ? (
                            <>
            ...
            <div className="flex justify-between items-center gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <Button
                    ...
                    variant="primary"
                    disabled={loading}
                    className="min-w-[200px] bg-yellow-600 hover:bg-yellow-700 text-white font-semibold"
                >
                    {loading ? (
                        <>
