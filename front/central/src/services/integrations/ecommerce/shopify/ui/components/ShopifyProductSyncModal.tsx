...
export function ShopifyProductSyncModal({ isOpen, onClose, integrationId, businessId, onCompleted }: ShopifyProductSyncModalProps) {
    ...
    const analyze = useCallback(async () => {
        ...
        setDiff({
            ...
            onlyInShopify: res.only_in_shopify || [],
            probabilityNoSku: Number(res.probability_no_sku) || 0,
            shopifyNoSku: Number(res.shopify_no_sku) || 0,
        });
        setPhase('diff');
    ...
    const handleApply = async (dir: Direction) => {
        setDirection(dir);
        setPhase('running');
        setTotal(dir === 'to_shopify' ? (diff?.onlyInProbability.length || 0) : (diff?.onlyInShopify.length || 0));
        setProcessed(0);
        setCreated(0);
        setFailed(0);
        correlationRef.current = null;
    ...
    const handleMessage = useCallback((event: MessageEvent) => {
        try {
            ...
            if (eventType === 'shopify.product.sync.started') {
                setTotal(Number(data.total) || 0);
            } else if (eventType === 'shopify.product.sync.progress') {
                setProcessed(Number(data.processed) || 0);
                setCreated(Number(data.created) || 0);
                setFailed(Number(data.failed) || 0);
            } else if (eventType === 'shopify.product.sync.completed') {
                if (data.error) {
    ...
    const inSync = diff && diff.onlyInProbability.length === 0 && diff.onlyInShopify.length === 0 && diff.matchedNotAssociated.length === 0;
                                ...
                                <>
                                        ...
                                        <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                                            <div className="flex items-start justify-between gap-3">
                                                ...
                                                <button onClick={() => handleApply('to_shopify')} className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg bg-emerald-600 hover:bg-emerald-700 px-3 py-1.5 text-xs font-semibold text-white transition-colors">
                                                    <ArrowUpFromLine size={14} /> Crear en Shopify
                                                </button>
                                            </div>
                                    ...
                                    {diff.onlyInShopify.length > 0 && (
                                        <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                                            ...
                                            <ProductList items={diff.onlyInShopify} />
                                        </div>
                                    )}
                                </>
                            )}

                            {(diff.probabilityNoSku > 0 || diff.shopifyNoSku > 0) && (
                                <p className="text-[11px] text-amber-600 dark:text-amber-400">
                                    Sin SKU (no se pueden cruzar): {diff.probabilityNoSku} en Probability, {diff.shopifyNoSku} en Shopify.
                                </p>
                            )}
                        </div>
                    )}
                    ...
                    {(phase === 'running' || phase === 'done') && (
                        <div>
                            <div className="flex items-center gap-2 mb-3 text-sm font-medium text-gray-700 dark:text-gray-200">
                                {direction === 'to_shopify' ? <ArrowUpFromLine size={16} className="text-emerald-600" /> : direction === null ? <Link2 size={16} className="text-amber-600" /> : <ArrowDownToLine size={16} className="text-blue-600" />}
                                {direction === 'to_shopify' ? 'Creando en Shopify' : direction === 'to_probability' ? 'Creando en Probability' : 'Asociando al canal'}
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-4">
                                <div className="flex items-center justify-between rounded-lg px-3 py-2 text-xs font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
                                    <span>Procesados</span><span className="tabular-nums">{created}</span>
                                </div>
