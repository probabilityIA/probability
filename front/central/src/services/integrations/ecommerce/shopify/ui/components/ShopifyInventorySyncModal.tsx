...
import { syncShopifyInventoryAction } from '../../infra/actions';

interface ShopifyInventorySyncModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const INVENTORY_EVENT_TYPES = ['shopify.inventory.sync.started', 'shopify.inventory.sync.progress', 'shopify.inventory.sync.completed'];
...
export function ShopifyInventorySyncModal({ isOpen, onClose, integrationId, businessId, onCompleted }: ShopifyInventorySyncModalProps) {
    ...
    const handleMessage = useCallback((event: MessageEvent) => {
        try {
            ...
            if (eventType === 'shopify.inventory.sync.started') {
                setTotal(Number(data.total) || 0);
                setPhase('running');
            } else if (eventType === 'shopify.inventory.sync.progress') {
            ...
            } else if (eventType === 'shopify.inventory.sync.completed') {
    ...
    useEffect(() => {
        ...
        const run = async () => {
            setPhase('starting');
            const result: any = await syncShopifyInventoryAction(integrationId, businessId ?? undefined);
            if (cancelled) return;
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                                <ArrowUpFromLine size={12} /> Probability &rarr; Shopify
