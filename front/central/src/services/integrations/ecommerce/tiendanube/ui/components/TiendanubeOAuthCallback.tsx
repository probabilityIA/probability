...
function readPendingConfig(): { cod_includes_shipping: boolean } {
    try {
        const raw = sessionStorage.getItem('tiendanube_pending_connection');
        if (raw) {
            const parsed = JSON.parse(raw);
            return { cod_includes_shipping: parsed?.cod_includes_shipping !== false };
        }
    } catch {
    }
    return { cod_includes_shipping: true };
}

export function TiendanubeOAuthCallback() {
    const router = useRouter();
    const searchParams = useSearchParams();
    const hasRun = useRef(false);
    const [status, setStatus] = useState<Status>('processing');
    const [message, setMessage] = useState('Conectando con Tiendanube...');
    ...
    useEffect(() => {
        ...
        const run = async () => {
            const oauthStatus = searchParams.get('tiendanube_oauth');

            if (oauthStatus === 'error') {
                setStatus('error');
                setMessage(searchParams.get('message') || 'Error al conectar con Tiendanube');
                return;
            }

            const integrationName = searchParams.get('integration_name') || 'Tiendanube';
            const integrationCode = `tiendanube_${Date.now()}`;
            ...
            try {
                ...
                const tokenResponse = await fetch(
                    `${apiBaseUrl}/integrations/tiendanube/oauth/token?state=${encodeURIComponent(state)}&exchange_token=${encodeURIComponent(exchangeToken)}`,
                    {
                ...
                if (!tokenResponse.ok || !tokenData.success) {
                    throw new Error(tokenData.error || 'No se pudieron recuperar las credenciales de Tiendanube');
                }
                ...
                const response = await createIntegrationAction({
                    ...
                    config: {
                        ...
                        cod_includes_shipping: pending.cod_includes_shipping,
                ...
                setStatus('success');
                setMessage('Tiendanube conectado exitosamente. Redirigiendo...');
