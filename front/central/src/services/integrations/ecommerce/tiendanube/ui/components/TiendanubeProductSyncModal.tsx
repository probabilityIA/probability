...
import { reconcileTiendanubeProductsAction, applyTiendanubeProductsAction, associateTiendanubeProductsAction } from '../../infra/actions';
import { fetchSyncRunItems } from '@/services/modules/my-integrations/infra/repository/sync-run-items';

interface TiendanubeProductSyncModalProps {
    isOpen: boolean;
    onClose: () => void;
...
interface Diff {
    ...
    onlyInTiendanube: Brief[];
    probabilityNoSku: number;
    tiendanubeNoSku: number;
}

const PRODUCT_EVENT_TYPES = [
    'tiendanube.product.reconcile.started',
    'tiendanube.product.reconcile.completed',
    'tiendanube.product.sync.started',
    'tiendanube.product.sync.progress',
    'tiendanube.product.sync.completed',
];
...
type Direction = 'to_tiendanube' | 'to_probability';
type Mode = 'create' | 'update';

export function TiendanubeProductSyncModal({ isOpen, onClose, integrationId, businessId, onCompleted }: TiendanubeProductSyncModalProps) {
    ...
    const aplicarConteos = useCallback(async (data: any) => {
        const [sinAsociar, soloProbability, soloTiendanube] = await Promise.all([
        ...
        setDiff({
            ...
            onlyInTiendanube: soloTiendanube,
            probabilityNoSku: Number(data.probability_no_sku) || 0,
            tiendanubeNoSku: Number(data.tiendanube_no_sku) || 0,
        });
        setPhase('diff');
    }, [cargarGrupo]);
    ...
    const analyze = useCallback(async () => {
        ...
        const res: any = await reconcileTiendanubeProductsAction(integrationId, businessId ?? undefined);
        if (!res?.success) {
        ...
        setDiff({
            ...
            onlyInTiendanube: res.only_in_tiendanube || [],
            probabilityNoSku: Number(res.probability_no_sku) || 0,
            tiendanubeNoSku: Number(res.tiendanube_no_sku) || 0,
        });
        setPhase('diff');
    }, [integrationId, businessId]);
    ...
    const handleApply = async (dir: Direction, opMode: Mode = 'create') => {
        ...
        if (opMode === 'update') {
            ...
            setTotal(dir === 'to_tiendanube' ? (diff?.onlyInProbability.length || 0) : (diff?.onlyInTiendanube.length || 0));
        }
        setProcessed(0);
        setCreated(0);
        setUpdated(0);
    ...
    const handleMessage = useCallback((event: MessageEvent) => {
        try {
            ...
            if (eventType === 'tiendanube.product.reconcile.started') return;

            if (eventType === 'tiendanube.product.reconcile.completed') {
            ...
            if (eventType === 'tiendanube.product.sync.started') {
                setTotal(Number(data.total) || 0);
            } else if (eventType === 'tiendanube.product.sync.progress') {
            ...
            } else if (eventType === 'tiendanube.product.sync.completed') {
    ...
    const inSync = diff && diff.onlyInProbability.length === 0 && diff.onlyInTiendanube.length === 0 && diff.matchedNotAssociated.length === 0;
                        ...
                        <div className="space-y-4">
                            ...
                            {inSync ? (
                                ...
                                <>
                                        ...
                                        <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                                            <div className="flex items-start justify-between gap-3">
                                                ...
                                                <button onClick={() => handleApply('to_tiendanube')} className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-lg bg-violet-600 hover:bg-violet-700 px-3 py-1.5 text-xs font-semibold text-white transition-colors">
                                                    <ArrowUpFromLine size={14} /> Crear en Tiendanube
                                                </button>
                                            </div>
                                    ...
                                    {diff.onlyInTiendanube.length > 0 && (
                                        <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                                            <div className="flex items-start justify-between gap-3">
                                                <div>
                                            ...
                                            <ProductList items={diff.onlyInTiendanube} />
                                        </div>
                                    )}
                                </>
                            )}
                            ...
                            <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
                                ...
                                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                                    <button
                                        ...
                                        title="Los SKU que son variantes no reciben nombre ni precio: en Tiendanube las variantes heredan del producto padre."
                                    ...
                                    >
                                        ...
                                        <span className="text-[11px] text-gray-500 dark:text-gray-400">Con los datos de Tiendanube: nombre, precio, peso y dimensiones.</span>
                                    </button>
                                </div>
                                <p className="text-[11px] text-gray-400 mt-2">Al actualizar en Tiendanube, los SKU que son variantes solo reciben peso y dimensiones del producto padre; se omiten nombre y precio.</p>
                            </div>

                            {(diff.probabilityNoSku > 0 || diff.tiendanubeNoSku > 0) && (
                                <p className="text-[11px] text-amber-600 dark:text-amber-400">
                                    Sin SKU (no se pueden cruzar): {diff.probabilityNoSku} en Probability, {diff.tiendanubeNoSku} en Tiendanube.
                                </p>
                            )}
                        </div>
                    )}
                    ...
                    {(phase === 'running' || phase === 'done') && (
                        <div>
                            <div className="flex items-center gap-2 mb-3 text-sm font-medium text-gray-700 dark:text-gray-200">
                                {direction === 'to_tiendanube' ? <ArrowUpFromLine size={16} className="text-violet-600" /> : direction === 'to_probability' ? <ArrowDownToLine size={16} className="text-blue-600" /> : <Link2 size={16} className="text-amber-600" />}
                                {direction === 'to_tiendanube'
                                    ? (mode === 'update' ? 'Actualizando en Tiendanube con los datos de Probability' : 'Creando en Tiendanube')
                                    : direction === 'to_probability'
                                        ? (mode === 'update' ? 'Actualizando en Probability con los datos de Tiendanube' : 'Creando en Probability')
                                        : 'Asociando al canal'}
