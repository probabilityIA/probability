...
interface TiendanubeOrderSyncModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const ORDER_EVENT_TYPES = [
    'tiendanube.orders.sync.started',
    'tiendanube.orders.sync.item',
    'tiendanube.orders.sync.progress',
    'tiendanube.orders.sync.completed',
    'tiendanube.orders.sync.failed',
];
...
export function TiendanubeOrderSyncModal({ isOpen, onClose, integrationId, businessId, createdAtMin, createdAtMax, onCompleted }: TiendanubeOrderSyncModalProps) {
    ...
    const handleMessage = useCallback((event: MessageEvent) => {
        try {
            ...
            if (!data) return;
            if (!eventType.startsWith('tiendanube.orders.sync.')) return;

            if (eventType === 'tiendanube.orders.sync.started') {
                setPhase('running');
            } else if (eventType === 'tiendanube.orders.sync.item') {
                setOrders((prev) => [...prev, {
            ...
            } else if (eventType === 'tiendanube.orders.sync.progress') {
            ...
            } else if (eventType === 'tiendanube.orders.sync.completed') {
