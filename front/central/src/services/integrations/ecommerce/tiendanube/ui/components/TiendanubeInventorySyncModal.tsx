...
import { syncTiendanubeInventoryAction } from '../../infra/actions';

interface TiendanubeInventorySyncModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const INVENTORY_EVENT_TYPES = [
    'tiendanube.inventory.sync.started',
    'tiendanube.inventory.sync.item',
    'tiendanube.inventory.sync.progress',
    'tiendanube.inventory.sync.completed',
];
...
export function TiendanubeInventorySyncModal({ isOpen, onClose, integrationId, businessId, onCompleted }: TiendanubeInventorySyncModalProps) {
    ...
    const handleMessage = useCallback((event: MessageEvent) => {
        try {
            ...
            if (eventType === 'tiendanube.inventory.sync.started') {
                setTotal(Number(data.total) || 0);
                setPhase('running');
            } else if (eventType === 'tiendanube.inventory.sync.item') {
                setItems((prev) => [...prev, {
                    sku: String(data.sku || ''),
                    quantity: Number(data.quantity) || 0,
                    action: data.action === 'failed' ? 'failed' : 'updated',
                }]);
            } else if (eventType === 'tiendanube.inventory.sync.progress') {
                setProcessed(Number(data.processed) || 0);
                setTotal(Number(data.total) || 0);
                setUpdated(Number(data.updated) || 0);
                setFailed(Number(data.failed) || 0);
            } else if (eventType === 'tiendanube.inventory.sync.completed') {
    ...
    useEffect(() => {
        ...
        const run = async () => {
            setPhase('starting');
            const result: any = await syncTiendanubeInventoryAction(integrationId, businessId ?? undefined);
            if (cancelled) return;
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden border border-gray-200 dark:border-gray-700 max-h-[90vh]">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                                <ArrowUpFromLine size={12} /> Probability &rarr; Tiendanube
                ...
                {(phase === 'starting' || phase === 'running' || phase === 'done') && (
                    <div className="px-6 py-5 overflow-y-auto">
                        ...
                        {items.length > 0 && (
                            <div className="mt-4">
                                <style>{`@keyframes tnInvRowIn{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:none}}`}</style>
                                ...
                                <div className="max-h-56 overflow-y-auto rounded-lg border border-gray-100 dark:border-gray-800 divide-y divide-gray-100 dark:divide-gray-800">
                                    {items.slice(-20).reverse().map((it, i) => (
                                        <div key={`${it.sku}-${i}`} style={{ animation: 'tnInvRowIn 0.25s ease' }} className="flex items-center justify-between gap-2 px-2.5 py-1.5 text-[11px]">
