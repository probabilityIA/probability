...
import { syncWooProductsAction } from '../../infra/actions';
...
export function WooProductSyncModal({ isOpen, onClose, integrationId, businessId, onCompleted }: WooProductSyncModalProps) {
    ...
    const [counts, setCounts] = useState<Counts>({ created: 0, updated: 0, failed: 0 });
    ...
    useEffect(() => {
        if (!isOpen) {
            ...
            setTotal(0);
            setProcessed(0);
            setCounts({ created: 0, updated: 0, failed: 0 });
        ...
        const run = async () => {
            setPhase('starting');
            const result: any = await syncWooProductsAction(integrationId, businessId ?? undefined);
            if (cancelled) return;
    ...
    const badges: Array<{ label: string; value: number; cls: string }> = [
        { label: 'Creados', value: counts.created, cls: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' },
        { label: 'Actualizados', value: counts.updated, cls: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300' },
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800 bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-950/40 dark:to-purple-950/40">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-violet-500/10 dark:bg-violet-400/10 flex items-center justify-center">
                            <RefreshCw size={18} className={`text-violet-600 dark:text-violet-400 ${busy ? 'animate-spin' : ''}`} />
                        </div>
                        <div>
                ...
                {(phase === 'starting' || phase === 'running' || phase === 'done') && (
                    <div className="px-6 py-5">
                        ...
                        <div className="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all duration-300 ease-out ${phase === 'done' ? 'bg-gradient-to-r from-violet-500 to-purple-500' : 'bg-gradient-to-r from-violet-400 to-purple-400'}`}
                        ...
                        {phase === 'starting' && (
                            <div className="flex items-center justify-center py-6 gap-2 text-gray-500 dark:text-gray-400">
                                ...
                                <span className="text-sm">Publicando productos a WooCommerce...</span>
                            </div>
                        )}
                        ...
                        {phase === 'done' && (
                            <div className="flex justify-end mt-5">
                                <button
                                    onClick={onClose}
                                    className="px-5 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-sm font-semibold transition-colors flex items-center gap-2"
                                >
