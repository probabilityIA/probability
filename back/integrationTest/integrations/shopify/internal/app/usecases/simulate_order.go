...
func (s *OrderSimulator) SimulateOrder(topic string) error {
	shopDomain := s.config.Get("SHOPIFY_SHOP_DOMAIN")
	if shopDomain == "" {
		return fmt.Errorf("SHOPIFY_SHOP_DOMAIN no configurado")
	}
...
func (s *OrderSimulator) CreateRandomOrder() (*domain.Order, error) {
	...
	s.orderRepository.Save(order)
	s.logger.Info().Str("order_number", orderNumber).Msg("Orden aleatoria creada")
	return order, nil
}
...
func (s *OrderSimulator) generateRandomOrder(orderNumber string) *domain.Order {
	...
	order := &domain.Order{
		...
		Name:                     orderNumber,
		Note:                     nil,
		NoteAttributes:           []domain.NoteAttribute{},
		Number:                   s.orderNumberSeq,
		OrderNumber:              s.orderNumberSeq,
		OrderStatusURL:           s.dataGenerator.stringPtr(fmt.Sprintf("https://%s/orders/%s/authenticate?key=abc", s.config.Get("SHOPIFY_SHOP_DOMAIN"), orderNumber)),
