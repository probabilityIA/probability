...
type WhatsAppConversation struct {
	...
	ConversationType string         `gorm:"type:varchar(20);not null;default:'order';check:conversation_type IN ('order','system_alert','inbound','campaign')"`
	BusinessID       uint           `gorm:"not null;index:idx_whatsapp_business_id"`
	CampaignID       *uint          `gorm:"index:idx_whatsapp_campaign_id"`
