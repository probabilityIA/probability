...
|-------|-----------|----------|---------|
| 2026-08-28 | `migrateUserTourProgress` | Crea `user_tour_progress` (user_id, business_id, tour_key, version, status, step_index, completed_at) con unico `(user_id, business_id, tour_key)`. Guarda el avance del product tour por usuario y por negocio: reemplaza el `localStorage` del tour de productos y habilita metricas de adopcion y drop-off por paso | local |
