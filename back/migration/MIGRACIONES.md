...
|-------|-----------|----------|---------|
| 2026-09-14 | `migrateAssistantConversations` | Crea `assistant_conversations` (id uuid, business_id, user_id, message_count, started_at, last_message_at) y `assistant_messages` (pregunta, respuesta, destino, error_code, modelo, tokens, latencia, feedback, clicked_at). Historial del asistente Via para revisar donde falla y cuanto cuesta (TKT-000093). Retencion de 1 ano la hace un job del modulo `ai`. Corrida en local y en produccion via `./scripts/run-migration-prod.sh` con `Migrate()` apuntando solo a esta migracion; el archivo `constructor.go` se restauro despues | local + produccion |
| 2026-09-08 | `migrateSyncRunItemGroup` | Agrega `group_ref`/`group_label` a `integration_sync_run_items` (+ indice en `group_ref`), paralela a `parent_ref`/`parent_label`. Segundo nivel de agrupacion (subfamilia) para el arbol madre/subfamilia/variante del panel de comparacion del Hub de integraciones. Corrida y verificada en local y en produccion el mismo dia via `./scripts/run-migration-prod.sh`, confirmada con `\d integration_sync_run_items` | local + produccion |
...
| 2026-09-09 | `migratePushNotifications` | Creo la tabla `device_tokens` y sembro el `notification_types` con code `push` (id 5) y sus 4 `notification_event_types`, para que el canal de notificaciones push exista como opcion configurable por negocio. Ademas resincroniza con `setval` las secuencias de `notification_types` y `notification_event_types`, que en produccion estaban atrasadas por filas insertadas con id explicito: el primer intento fallo con `duplicate key value violates unique constraint notification_types_pkey` | produccion |
| 2026-09-09 | `migrateConfirmacionConMapa` | Sembro el `notification_event_type` `order.created_with_map` para el canal WhatsApp (id 19 en produccion): la variante de la confirmacion contra entrega que adjunta un mapa de la direccion para que el cliente valide su ubicacion antes de generar la guia. Es opt-in por negocio desde el modulo de notificaciones; si nadie la activa, el flujo sigue igual | produccion |
| 2026-09-09 | `migrateEstadosConfirmacionMapa` | Copio al evento `order.created_with_map` los mismos `notification_event_type_allowed_statuses` del evento original `order.created` de WhatsApp (pending, processing, picking). Sin esa lista la UI no ofrecia ningun estado que marcar y la regla quedaba sin poder configurarse: la celda mostraba 'Sin filtro de estado' y no habia chips para tocar | produccion |

| 2026-09-13 | `migrateWhatsappTemplateHeaderMedia` + `migrateWhatsappTemplateFlows` + `migrateWhatsappFlows` + `migrateWhatsappCampaignSchedule` | Esquema de flujos de WhatsApp y programacion de campanas, antes del deploy que los usa: `header_type`/`header_media_url`/`header_media_handle` en `whatsapp_templates` (backfill `header_type='TEXT'`), tablas `whatsapp_template_flows` y `whatsapp_flows` con sus indices unicos, `flow_id` en `whatsapp_campaigns` y `scheduled_notification_rules`, y en campanas `schedule_mode`, `interval_days`, `send_dates`, `delivery_mode`, `occurrences`, `current_round`. En `whatsapp_campaign_sends` agrega `round` y reemplaza el unico `(campaign_id, client_id)` por `(campaign_id, round, client_id)`. Produccion tenia 0 campanas y 0 tareas programadas al correrla. Solo esas cuatro, no la cadena | produccion |

| 2026-09-13 | `migrateWhatsappConversationReads` + `migrateSystemTemplateBodies` + `migrateWhatsappMessageMedia` | Bandeja de conversaciones: crea `whatsapp_conversation_reads` (marca de leido por negocio y telefono) y la rellena con NOW() para los 975 telefonos existentes, para que el historial no aparezca como no leido; llena `body_text`, `header_text`, `footer_text` y `buttons` de las 26 plantillas del sistema con el texto aprobado en Meta (solo donde estaban vacios; 7 quedan con botones); agrega `media_type`, `media_key`, `media_mime`, `media_filename`, `media_size` a `whatsapp_message_logs` para adjuntos del chat. Verificado: 975 lecturas, 0 plantillas del sistema sin cuerpo, 5 columnas de media. Solo esas tres, no la cadena | produccion |

| 2026-09-14 | `MigrateCodCheckoutCarrierFee` + `FixCodTotalCheckoutWoo` | **TKT-000088**. Agrega `orders.cod_checkout_carrier_fee` (numeric(12,2), default 0): la comision del carrier que el comprador ya pago en el checkout del plugin de WooCommerce. Orden de la corrida: DDL, deploy de `8c547183`, DML. El DML separa la comision del `cod_total` en las ordenes del plugin con `cod = 1`: 63 ordenes (45 Viga, 18 Demo), 61 restan la comision y 14679/15765 ya estaban netas. Excluidas las que no cuadran con el total de la tienda o estan en un corte. Verificado: 62/63 cumplen `cod_total + comision = total de la tienda` (14679 editada a mano). DML borrado del repo tras la corrida. Bitacora `2026-09-14-cod-total-mezclaba-comision-checkout-woocommerce.md` | local + produccion |

| 2026-09-14 | `MigrateShipmentCodCollectAmount` + `FixShipmentCodCollectAmount` + `FixShipmentCodCollectAmountVigaPDF` | **TKT-000088**. Agrega `shipments.cod_collect_amount` (numeric(12,2), nullable): el valor a recaudar que se le declaro a EnvioClick al generar la guia, que es lo que imprime el PDF. Orden de la corrida: DDL y DML antes del deploy (el codigo desplegado no lee la columna). DML 1: rellena desde `shipment_sync_logs` (ultimo `/api/v2/shipment` exitoso con `codValue`): 91 envios en produccion, 3 en local. DML 2: Viga (negocio 46), guias sin log, valor leido del PDF de cada guia (Coordinadora "Valor a recaudar", Envia "Valor a recaudar $", Interrapidisimo "Valor a cobrar $"): 63 envios, 55 de ellos en cortes confirmados (no cambia lo pagado, el corte usa `cod_total`). Verificado: Viga 144/145 guias con valor; VIG-0122 queda sin valor porque su guia no es contra entrega ($0). Corrida solo de esas funciones con un cmd temporal, no la cadena. DML borrados del repo tras la corrida | local + produccion |
...
| 2026-09-16 | `migrateAssistantAlerts` | Crea `assistant_alerts` (alertas por negocio con event_id unico, titulo, cuerpo, destino, referencia) y `assistant_alert_cursors` (ultimo visto por usuario y negocio). Inserta el canal de notificacion `assistant` ("Via") con **id 6 fijo** (el dispatcher de `services/events` lo espera asi; la migracion falla si queda con otro id) y sus 8 tipos de evento. Corrida en local (TKT-000096). Corrida en produccion antes del deploy | local + produccion (2026-09-16) |
| 2026-09-16 | `migrateCancelRequestedStatus` | Inserta el estado de orden `cancel_requested` ("Cliente solicita cancelar", categoria issue, color #F59E0B) y registra la plantilla del sistema `solicitud_cancelacion_recibida` (meta_template_id 1782177306268808, aprobada en Meta). Corre ANTES de `migrateAssistantAlerts` porque esa agrega el estado a los permitidos de Via (TKT-000099) | local + produccion (2026-09-16) |
| 2026-09-16 | `migrateDefaultNotificationRules` | Backfill: a cada negocio con integracion Plataforma le crea la regla Via + "Cambio de estado de la orden" con estados cancelada y cliente solicita cancelar; si ya existe, la activa y le deja exactamente esos dos estados. Corre DESPUES de `migrateAssistantAlerts`. En prod hay 21 negocios y 20 con Plataforma (TKT-000099) | local + produccion (2026-09-16) |
