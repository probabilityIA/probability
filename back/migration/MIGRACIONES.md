...
|-------|-----------|----------|---------|
| 2026-09-04 | `migrateSubscriptionAutoPayment` | Agrega `subscription_auto_payment_enabled` a `business` (default `false`): toggle del negocio para que la suscripcion se pague sola desde la billetera el dia que vence, si hay saldo. Lo lee `ListBusinessesJustExpired`/`autoRenewIfEnabled` en el worker de expiracion, antes de aplicar el corte | local |
