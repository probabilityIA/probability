...
func (h *Handlers) CreateCustomPlan(c *gin.Context) {
	...
	subType, err := h.uc.CreateCustomPlan(c.Request.Context(), dtos.CreateCustomPlanDTO{
		...
		IncludedTemplateMessages:    req.IncludedTemplateMessages,
		TemplateMessageOveragePrice: req.TemplateMessageOveragePrice,
		IncludedTemplates:           req.IncludedTemplates,
		TemplateOveragePrice:        req.TemplateOveragePrice,
	}, c.GetUint("user_id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
