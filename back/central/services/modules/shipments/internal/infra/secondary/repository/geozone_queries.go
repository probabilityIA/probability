...
func (r *Repository) ResolveShipmentGeozone(ctx context.Context, shipmentID uint, businessID uint) error {
	return r.db.Conn(ctx).Exec(`
		...
		chain AS (
		    SELECT id, parent_id, ARRAY[id]::bigint[] AS path FROM geozones WHERE id = (SELECT id FROM match)
		    UNION ALL
		    SELECT g.id, g.parent_id, c.path || g.id
		...
		SET destination_point = (SELECT p::geography FROM src),
		    ...
		    destination_geozone_path = COALESCE(
		        (SELECT to_jsonb(path) FROM chain ORDER BY array_length(path, 1) DESC LIMIT 1),
		        destination_geozone_path
		    )
		WHERE id = ?
