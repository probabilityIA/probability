...
func (r *Repository) ResolveOrderGeozone(ctx context.Context, orderID string, businessID uint) error {
	...
	return r.db.Conn(ctx).Exec(`
		WITH RECURSIVE target AS (
		    SELECT id, shipping_city, shipping_state
		...
		matched_city AS (
		    ...
		    JOIN geozones g
		     ...
		     AND (g.business_id = 0 OR g.business_id = ?)
		     AND unaccent(lower(g.name)) = unaccent(lower(t.shipping_city))
		    LIMIT 1
		),
		matched_state AS (
		    SELECT t.id AS order_id, g.id AS gid
		    ...
		    JOIN geozones g
		     ...
		     AND (g.business_id = 0 OR g.business_id = ?)
		     AND unaccent(lower(g.name)) = unaccent(lower(COALESCE(t.shipping_state, '')))
		    LIMIT 1
		),
		picked AS (
	...
	`, orderID, businessID, businessID, orderID).Error
}
