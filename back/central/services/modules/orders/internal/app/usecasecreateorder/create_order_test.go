...
type mockRepository struct {
	...
	GetBusinessNameByIDFn                                func(ctx context.Context, businessID uint) (string, error)
...
func (m *mockRepository) GetBusinessNameByID(ctx context.Context, businessID uint) (string, error) {
	if m.GetBusinessNameByIDFn != nil {
		return m.GetBusinessNameByIDFn(ctx, businessID)
	}
...
func (m *mockRepository) BusinessHasWarehouse(ctx context.Context, businessID uint) (bool, error) {
	if m.BusinessHasWarehouseFn != nil {
...
func (m *mockRepository) UpdateOrderStatusWithSource(ctx context.Context, orderID string, status string, statusID *uint, source string, changedBy string) error {
	return nil
}
...
func (m *mockUpdateUseCase) UpdateOrder(ctx context.Context, existingOrder *entities.ProbabilityOrder, dto *dtos.ProbabilityOrderDTO) (*dtos.OrderResponse, error) {
	if m.UpdateOrderFn != nil {
		return m.UpdateOrderFn(ctx, existingOrder, dto)
	}
...
func newTestCreateUseCase(
	...
	if updateUseCase != nil {
		uc.updateUseCase = updateUseCase
	}
	return uc
}
...
func TestMapAndSaveOrder_NegocioEliminado_RetornaErrorDeDominio(t *testing.T) {
	repo := &mockRepository{
		GetBusinessNameByIDFn: func(ctx context.Context, businessID uint) (string, error) {
			return "", nil
		},
	...
	if err == nil {
		t.Fatal("se esperaba error cuando el negocio esta eliminado, pero no se obtuvo ninguno")
	}
	if result != nil {
		t.Errorf("se esperaba resultado nil, pero se obtuvo: %v", result)
	}
	if !errors.Is(err, domainerrors.ErrOrderBusinessDeleted) {
		t.Errorf("se esperaba ErrOrderBusinessDeleted, pero se obtuvo: %v", err)
	}
}
...
func TestMapAndSaveOrder_OrdenExistente_LlamaUpdateOrder(t *testing.T) {
	...
	mockUpdate := &mockUpdateUseCase{
		UpdateOrderFn: func(ctx context.Context, existing *entities.ProbabilityOrder, dto *dtos.ProbabilityOrderDTO) (*dtos.OrderResponse, error) {
