...
func (uc *UseCaseOrder) UpdateOrder(ctx context.Context, id string, req *dtos.UpdateOrderRequest) (*dtos.OrderResponse, error) {
	...
	if req.ShippingOfficeName != nil {
		order.ShippingOfficeName = *req.ShippingOfficeName
	}
	if req.ShippingOfficeDetails != nil {
		order.ShippingOfficeDetails = req.ShippingOfficeDetails
	}
	...
	if len(req.Items) > 0 {
		...
		uc.linkItemsAndComputePackage(ctx, order, domainItems)
