...
func (h *handler) RegisterRoutes(router *gin.RouterGroup) {
	...
	router.GET("/ecommerce-integration-types", middleware.JWT(), h.ListEcommerceIntegrationTypes) // GET /api/v1/ecommerce-integration-types

	// Rutas para estados de canales de integración
	channelStatuses := router.Group("/channel-statuses")
	channelStatuses.Use(middleware.JWT())
	{
	...
	fulfillmentStatuses.Use(middleware.JWT())
	{
