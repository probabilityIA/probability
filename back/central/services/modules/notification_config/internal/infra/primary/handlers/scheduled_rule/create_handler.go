...
func (h *handler) Create(c *gin.Context) {
	...
	var body dtos.CreateScheduledRuleDTO
	...
	rule, err := h.useCase.CreateRule(c.Request.Context(), body)
	if err != nil {
		h.logger.Error().Err(err).Str("name", body.Name).Msg("Error creando regla programada")
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": err.Error()})
		return
	}
