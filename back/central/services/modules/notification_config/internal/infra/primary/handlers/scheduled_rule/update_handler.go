...
func (h *handler) Update(c *gin.Context) {
	...
	var body dtos.UpdateScheduledRuleDTO
	...
	rule, err := h.useCase.UpdateRule(c.Request.Context(), body)
	if err != nil {
		h.logger.Error().Err(err).Uint("id", id).Msg("Error actualizando regla programada")
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": err.Error()})
		return
	}
