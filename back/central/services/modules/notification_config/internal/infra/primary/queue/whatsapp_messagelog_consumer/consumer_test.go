...
func TestStart_DeclaresQueueBeforeConsuming(t *testing.T) {
	...
	if declaredQueue != rabbitmq.QueueWhatsAppMessageLogEvents {
		t.Errorf("expected declared queue %s, got %s", rabbitmq.QueueWhatsAppMessageLogEvents, declaredQueue)
	}
	if consumedQueue != rabbitmq.QueueWhatsAppMessageLogEvents {
		t.Errorf("expected consumed queue %s, got %s", rabbitmq.QueueWhatsAppMessageLogEvents, consumedQueue)
	}
