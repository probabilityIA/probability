...
func (h *Handlers) CutEmailHistory(c *gin.Context) {
	businessID, err := resolveBusinessID(c)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": err.Error()})
		return
	...
	logs, err := h.uc.CutEmailHistory(c.Request.Context(), businessID, uint(cutID))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
			"message": "Error al obtener el historico de correos del corte",
			"error":   err.Error(),
		})
		return
	}

	c.JSON(http.StatusOK, gin.H{"success": true, "data": mapCutEmailLogs(logs)})
}
