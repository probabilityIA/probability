...
func (h *Handlers) GetWarehouseTree(c *gin.Context) {
	...
	tree, err := h.uc.GetWarehouseTree(c.Request.Context(), businessID, uint(warehouseID))
	if err != nil {
	...
	c.JSON(http.StatusOK, response.WarehouseTreeFromDTO(tree))
}
