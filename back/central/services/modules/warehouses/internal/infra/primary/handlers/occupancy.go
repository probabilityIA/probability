...
func (h *Handlers) GetOccupancy(c *gin.Context) {
	...
	items, err := h.uc.GetOccupancy(c.Request.Context(), businessID, uint(warehouseID))
	if err != nil {
	...
	c.JSON(http.StatusOK, response.OccupancyFromEntities(uint(warehouseID), items))
}
