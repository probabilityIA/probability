...
// ActivateIntegrationHandler activa una integración
//
//	@Summary		Activar integración
//	@Description	Activa una integración del sistema
//	@Tags			Integrations
//	@Accept			json
//	@Produce		json
//	@Security		BearerAuth
...
func (h *IntegrationHandler) ActivateIntegrationHandler(c *gin.Context) {
	if !middleware.IsSuperAdmin(c) {
		h.logger.Error().Str("endpoint", "/integrations/:id/activate").Str("method", "PUT").Msg("Intento de activar integración sin permisos de super admin")
	...
	if err != nil {
		h.logger.Error().Err(err).Str("id", idStr).Str("endpoint", "/integrations/:id/activate").Str("method", "PUT").Msg("ID de integración inválido al intentar activar")
	...
	if err := h.usecase.ActivateIntegration(c.Request.Context(), uint(id)); err != nil {
		statusCode := http.StatusInternalServerError
		errorMsg := "Error al activar integración"
		...
		h.logger.Error().
			...
			Msg("Error al activar integración en el usecase")
