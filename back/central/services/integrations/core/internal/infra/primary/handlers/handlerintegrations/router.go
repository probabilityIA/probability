...
func (h *IntegrationHandler) RegisterRoutes(router *gin.RouterGroup, logger log.ILogger) {
	integrationsGroup := router.Group("/integrations")
	{
		...
		integrationsGroup.GET("/:id/product-match", middleware.JWT(), h.GetProductMatchConfigHandler)
		integrationsGroup.PUT("/:id/product-match", middleware.JWT(), h.UpdateProductMatchConfigHandler)
