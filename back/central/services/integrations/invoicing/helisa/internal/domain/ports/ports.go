...
type IHelisaClient interface {
	...
	TestAuthentication(ctx context.Context, username, password, companyID, baseURL string) error
...
type IInvoiceUseCase interface {
	...
	// contra la API de Helisa. Llamado desde el contrato global IIntegrationContract.
