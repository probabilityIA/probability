...
func New(
	...
	return &InvoiceRequestConsumer{
		...
		log:               logger.WithModule("helisa.invoice_request_consumer"),
	}
}
...
func (c *InvoiceRequestConsumer) Start(ctx context.Context) error {
	...
	c.log.Info(ctx).
		Str("queue", QueueHelisaRequests).
		Msg("Starting Helisa invoice request consumer")

	if err := c.rabbit.DeclareQueue(QueueHelisaRequests, true); err != nil {
		c.log.Error(ctx).Err(err).Msg("Failed to declare queue")
		return err
	}

	if err := c.rabbit.Consume(ctx, QueueHelisaRequests, c.handleInvoiceRequest); err != nil {
	...
	c.log.Info(ctx).
		Str("queue", QueueHelisaRequests).
		Msg("Helisa consumer started successfully")
...
func (c *InvoiceRequestConsumer) handleInvoiceRequest(message []byte) error {
	...
	c.log.Info(ctx).
		...
		Msg("Received Helisa invoice request")
...
// processCreateInvoice procesa la creación de una factura en Helisa
// TODO: Implementar lógica real de creación de factura con la API de Helisa
func (c *InvoiceRequestConsumer) processCreateInvoice(
	...
	c.log.Warn(ctx).
		...
		Msg("⚠️ Helisa invoice creation not yet implemented")
	return c.createErrorResponse(request, "not_implemented", "helisa: invoice creation not yet implemented", startTime)
}
