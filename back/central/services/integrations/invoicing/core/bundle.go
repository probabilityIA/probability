...
const (
	...
	// modules/invoicing publica aquí; el core decide a qué facturador enrutar.
	QueueInvoicingRequests = "invoicing.requests"

	// Colas de cada proveedor (lectura de los consumers de softpymes/factus/siigo)
...
// al consumer del proveedor correcto (softpymes / factus).
type Bundle struct {
...
// los bundles de softpymes y factus.
func New(
	logger log.ILogger,
	rabbit rabbitmq.IQueue,
) *Bundle {
	logger = logger.WithModule("invoicing.core")
	...
	if rabbit == nil {
		logger.Warn(context.Background()).
			Msg("❌ RabbitMQ no disponible, router de facturación (invoicing.core) deshabilitado")
		return b
	}

	// Iniciar router en goroutine (mismo patrón que softpymes/factus bundles)
	go func() {
		ctx := context.Background()
		logger.Info(ctx).Msg("🚀 Starting invoicing core router in background...")
		if err := b.startRouter(ctx); err != nil {
			logger.Error(ctx).Err(err).Msg("❌ Invoicing core router failed to start or stopped with error")
		}
	}()

	logger.Info(context.Background()).Msg("✅ Invoicing core bundle initialized")
...
func (b *Bundle) startRouter(ctx context.Context) error {
	...
	for _, q := range []string{QueueSoftpymesRequests, QueueFactusRequests, QueueSiigoRequests} {
	...
	b.log.Info(ctx).
		...
		Msg("✅ Invoicing core router listening")
