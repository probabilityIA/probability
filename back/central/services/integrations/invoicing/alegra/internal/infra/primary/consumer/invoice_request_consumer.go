...
func NewInvoiceRequestConsumer(
	...
	return &InvoiceRequestConsumer{
		...
		log:               logger.WithModule("alegra.invoice_request_consumer"),
	}
}
...
func (c *InvoiceRequestConsumer) Start(ctx context.Context) error {
	...
	c.log.Info(ctx).
		Str("queue", QueueAlegraRequests).
		Msg("Starting Alegra invoice request consumer")

	if err := c.rabbit.DeclareQueue(QueueAlegraRequests, true); err != nil {
		c.log.Error(ctx).Err(err).Msg("Failed to declare queue")
		return err
	}

	if err := c.rabbit.Consume(ctx, QueueAlegraRequests, c.handleInvoiceRequest); err != nil {
	...
	c.log.Info(ctx).
		Str("queue", QueueAlegraRequests).
		Msg("Alegra consumer started successfully")
...
func (c *InvoiceRequestConsumer) handleInvoiceRequest(message []byte) error {
	...
	c.log.Info(ctx).
		...
		Msg("Received Alegra invoice request")
...
// processCreateInvoice procesa la creación de una factura en Alegra
// TODO: Implementar lógica real de creación de factura con la API de Alegra
func (c *InvoiceRequestConsumer) processCreateInvoice(
	...
	c.log.Warn(ctx).
		...
		Msg("⚠️ Alegra invoice creation not yet implemented")
	return c.createErrorResponse(request, "not_implemented", "alegra: invoice creation not yet implemented", startTime)
}
