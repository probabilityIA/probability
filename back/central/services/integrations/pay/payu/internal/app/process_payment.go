...
// PaymentRequestMsg mensaje recibido desde la cola pay.payu.requests
...
// ProcessPayment procesa una solicitud de pago via PayU
func (uc *useCase) ProcessPayment(ctx context.Context, msg *PaymentRequestMsg) error {
	...
	uc.log.Info(ctx).
		...
		Msg("Processing PayU payment")

	config, err := uc.integrationRepo.GetPayUConfig(ctx)
	if err != nil {
		uc.log.Error(ctx).Err(err).Msg("Failed to get PayU config")
	...
	transactionID, redirectURL, err := uc.payuClient.CreateTransaction(ctx, config, msg.Amount, currency, msg.Reference, msg.Description)
	if err != nil {
		uc.log.Error(ctx).Err(err).Msg("PayU transaction creation failed")
		return uc.publishError(ctx, msg, "api_error", err.Error(), startTime)
	}

	uc.log.Info(ctx).
		Uint("transaction_id", msg.PaymentTransactionID).
		Str("payu_tx_id", transactionID).
		Msg("PayU transaction created successfully")
