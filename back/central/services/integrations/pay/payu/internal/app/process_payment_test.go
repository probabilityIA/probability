...
import (
	...
	"github.com/secamc93/probability/back/central/services/integrations/pay/payu/internal/mocks"
)
...
func TestProcessPayment_PublicaLaRespuestaExitosaConElIDExterno(t *testing.T) {
	...
	assert.Equal(t, "payu", resp.GatewayCode)
	assert.EqualValues(t, 501, resp.PaymentTransactionID)
	require.NotNil(t, resp.ExternalID)
	assert.Equal(t, "TRX-1", *resp.ExternalID,
		"el id externo es lo que permite conciliar despues contra la pasarela")
	assert.Equal(t, "TRX-1", resp.GatewayResponse["transaction_id"])
	assert.Equal(t, "https://checkout.payulatam.com/TRX-1", resp.GatewayResponse["redirect_url"])
...
func TestProcessPayment_SinCredencialesPublicaErrorDeConfiguracion(t *testing.T) {
	e := nuevoEntorno()
	e.repo.GetPayUConfigFn = func(ctx context.Context) (*ports.PayUConfig, error) {
...
func TestProcessPayment_SiLaPasarelaFallaPublicaErrorDeAPI(t *testing.T) {
	e := nuevoEntorno()
	e.client.CreateTransactionFn = func(ctx context.Context, config *ports.PayUConfig, amount float64, currency, reference, description string) (string, string, error) {
