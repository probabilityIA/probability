...
type IntegrationRepositoryMock struct {
	GetPayUConfigFn func(ctx context.Context) (*ports.PayUConfig, error)
...
func (m *IntegrationRepositoryMock) GetPayUConfig(ctx context.Context) (*ports.PayUConfig, error) {
	m.Consultas++
	if m.GetPayUConfigFn != nil {
		return m.GetPayUConfigFn(ctx)
	}
	return &ports.PayUConfig{Environment: "sandbox"}, nil
}

type ClientMock struct {
	CreateTransactionFn func(ctx context.Context, config *ports.PayUConfig, amount float64, currency, reference, description string) (string, string, error)
...
type Llamada struct {
	...
	Reference   string
	Description string
	Config      *ports.PayUConfig
}

var _ ports.IPayUClient = (*ClientMock)(nil)

func (m *ClientMock) CreateTransaction(ctx context.Context, config *ports.PayUConfig, amount float64, currency, reference, description string) (string, string, error) {
	...
	return "TRX-1", "https://checkout.payulatam.com/TRX-1", nil
}
