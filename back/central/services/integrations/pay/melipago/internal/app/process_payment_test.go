...
import (
	...
	"github.com/secamc93/probability/back/central/services/integrations/pay/melipago/internal/mocks"
)
...
func TestProcessPayment_PublicaLaRespuestaExitosaConElIDExterno(t *testing.T) {
	...
	assert.Equal(t, "melipago", resp.GatewayCode)
	assert.EqualValues(t, 501, resp.PaymentTransactionID)
	require.NotNil(t, resp.ExternalID)
	assert.Equal(t, "PREF-1", *resp.ExternalID,
		"el id externo es lo que permite conciliar despues contra la pasarela")
	assert.Equal(t, "PREF-1", resp.GatewayResponse["preference_id"])
	assert.Equal(t, "https://mercadopago.com/PREF-1", resp.GatewayResponse["checkout_url"])
...
func TestProcessPayment_SinCredencialesPublicaErrorDeConfiguracion(t *testing.T) {
	e := nuevoEntorno()
	e.repo.GetMeliPagoConfigFn = func(ctx context.Context) (*ports.MeliPagoConfig, error) {
...
func TestProcessPayment_SiLaPasarelaFallaPublicaErrorDeAPI(t *testing.T) {
	e := nuevoEntorno()
	e.client.CreatePreferenceFn = func(ctx context.Context, config *ports.MeliPagoConfig, amount float64, currency, reference, description string) (string, string, error) {
