...
import (
	...
	"github.com/secamc93/probability/back/central/services/integrations/pay/wompi/internal/mocks"
)
...
func TestProcessPayment_PublicaLaRespuestaExitosaConElIDExterno(t *testing.T) {
	...
	assert.Equal(t, "wompi", resp.GatewayCode)
	...
	assert.Equal(t, "https://checkout.wompi.co/TRX-1", resp.GatewayResponse["redirect_url"])
...
func TestProcessPayment_SinCredencialesPublicaErrorDeConfiguracion(t *testing.T) {
	e := nuevoEntorno()
	e.repo.GetWompiConfigFn = func(ctx context.Context) (*ports.WompiConfig, error) {
...
func TestProcessPayment_SiLaPasarelaFallaPublicaErrorDeAPI(t *testing.T) {
	e := nuevoEntorno()
	e.client.CreateTransactionFn = func(ctx context.Context, config *ports.WompiConfig, amount float64, currency, reference, description string) (string, string, error) {
