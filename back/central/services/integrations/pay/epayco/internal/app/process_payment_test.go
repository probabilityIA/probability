...
import (
	...
	"github.com/secamc93/probability/back/central/services/integrations/pay/epayco/internal/mocks"
)
...
func TestProcessPayment_PublicaLaRespuestaExitosaConElIDExterno(t *testing.T) {
	...
	assert.Equal(t, "epayco", resp.GatewayCode)
	assert.EqualValues(t, 501, resp.PaymentTransactionID)
	require.NotNil(t, resp.ExternalID)
	assert.Equal(t, "CHK-1", *resp.ExternalID,
		"el id externo es lo que permite conciliar despues contra la pasarela")
	assert.Equal(t, "CHK-1", resp.GatewayResponse["checkout_id"])
	assert.Equal(t, "https://checkout.epayco.co/CHK-1", resp.GatewayResponse["redirect_url"])
...
func TestProcessPayment_SinCredencialesPublicaErrorDeConfiguracion(t *testing.T) {
	e := nuevoEntorno()
	e.repo.GetEPaycoConfigFn = func(ctx context.Context) (*ports.EPaycoConfig, error) {
...
func TestProcessPayment_SiLaPasarelaFallaPublicaErrorDeAPI(t *testing.T) {
	e := nuevoEntorno()
	e.client.CreateCheckoutFn = func(ctx context.Context, config *ports.EPaycoConfig, amount float64, currency, reference, description string) (string, string, error) {
