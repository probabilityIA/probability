...
// PaymentRequestMsg mensaje recibido desde la cola pay.epayco.requests
...
// ProcessPayment procesa una solicitud de pago via ePayco
func (uc *useCase) ProcessPayment(ctx context.Context, msg *PaymentRequestMsg) error {
	...
	uc.log.Info(ctx).
		...
		Msg("Processing ePayco payment")

	config, err := uc.integrationRepo.GetEPaycoConfig(ctx)
	if err != nil {
		uc.log.Error(ctx).Err(err).Msg("Failed to get ePayco config")
	...
	checkoutID, redirectURL, err := uc.epaycoClient.CreateCheckout(ctx, config, msg.Amount, currency, msg.Reference, msg.Description)
	if err != nil {
		uc.log.Error(ctx).Err(err).Msg("ePayco checkout creation failed")
		return uc.publishError(ctx, msg, "api_error", err.Error(), startTime)
	}

	uc.log.Info(ctx).
		Uint("transaction_id", msg.PaymentTransactionID).
		Str("checkout_id", checkoutID).
		Msg("ePayco checkout created successfully")
