...
const (
	webhookSource          = "bancolombia"
	integrationTypeCodeRaw = "bancolombia_qr"
	webhookRetentionDays   = 15
	webhookSignatureHeader = "x-bancolombia-signature"
)
...
func NewRawWebhookLogRepository(database db.IDatabase, logger log.ILogger) ports.IRawWebhookLogger {
	return &RawWebhookLogRepository{
		db:  database,
		log: logger.WithModule("bancolombia.raw_webhook_log_repository"),
	}
}
...
func (r *RawWebhookLogRepository) LogIncoming(ctx context.Context, raw *ports.RawWebhookLog) error {
	...
	path := "/api/v1/webhooks/bancolombia"
	if raw.Endpoint == "test" {
		path = "/api/v1/webhooks/bancolombia/test"
	}

	headers := map[string]string{}
	if raw.SignatureHeader != "" {
		headers[webhookSignatureHeader] = raw.SignatureHeader
	}
	...
	code := integrationTypeCodeRaw
	retentionUntil := time.Now().Add(webhookRetentionDays * 24 * time.Hour)
