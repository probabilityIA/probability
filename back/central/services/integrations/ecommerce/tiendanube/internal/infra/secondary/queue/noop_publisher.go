...
// NewNoOpPublisher crea un publisher que descarta las ordenes (cuando RabbitMQ no esta disponible).
...
func (p *noOpPublisher) Publish(ctx context.Context, order *canonical.ProbabilityOrderDTO) error {
	p.logger.Warn(ctx).
		...
		Msg("RabbitMQ not available, Tiendanube order not published to queue (no-op)")
	return nil
}
