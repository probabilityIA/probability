...
const (
	channelLabel              = "Tiendanube"
	reconcileStartedEventType = "tiendanube.product.reconcile.started"
	reconcileEventType        = "tiendanube.product.reconcile.completed"
)
...
func (uc *tiendanubeUseCase) ReconcileProductsAsync(ctx context.Context, integrationID string, businessID, integIDUint uint, correlationID string) {
	uc.emitSyncEvent(ctx, businessID, integIDUint, reconcileStartedEventType, map[string]interface{}{"correlation_id": correlationID})

	result, err := uc.ReconcileProducts(ctx, integrationID, businessID)
	if err != nil {
		uc.logger.Error(ctx).Err(err).Str("integration_id", integrationID).Msg("Error al comparar el catalogo con Tiendanube")
...
func reconcileDetail(result *domain.ReconcileResult) []reconcileDetailItem {
	...
	add(result.OnlyInTiendanube, "solo en "+channelLabel, "warn", "only_channel")

	return detail
}

func (uc *tiendanubeUseCase) publishReconcileRun(ctx context.Context, businessID, integrationID uint, data map[string]interface{}) {
	if uc.rabbit == nil {
		return
	}
