...
func (uc *SyncOrdersUseCase) ProcessOrderCancelled(ctx context.Context, shopDomain string, order *domain.ShopifyOrder) error {
	if order == nil {
	...
	uc.log.Info(ctx).
		...
		Msg("Processing orders/cancelled webhook")
