...
func ordenMinima() *domain.ProbabilityOrderDTO {
	return &domain.ProbabilityOrderDTO{
		OccurredAt: momento(),
		ImportedAt: momento(),
	}
}
...
func TestMapDomainToSerializable_ConservaLosCamposDePresentmentPropiosDeShopify(t *testing.T) {
	orden := ordenMinima()
	orden.CurrencyPresentment = "USD"
	orden.TotalAmountPresentment = 15.5
	orden.ShippingDiscount = 3000
	orden.IsTest = true

	out := MapDomainToSerializable(orden)

	assert.Equal(t, "USD", out.CurrencyPresentment,
		"Shopify maneja moneda de presentacion aparte de la de la tienda; el mapper canonico compartido no tiene estos campos")
	assert.InDelta(t, 15.5, out.TotalAmountPresentment, 0.001)
	assert.InDelta(t, 3000, out.ShippingDiscount, 0.001)
	assert.True(t, out.IsTest,
		"la bandera de orden de prueba debe sobrevivir el viaje: si se pierde, una orden de sandbox se procesa como real")
}
...
func TestMapDomainToSerializable_TrasladaLosItemsConSusPrecios(t *testing.T) {
	orden := ordenMinima()
	orden.OrderItems = []domain.ProbabilityOrderItemDTO{
...
func TestMapDomainToSerializable_TrasladaLasDirecciones(t *testing.T) {
	orden := ordenMinima()
	orden.Addresses = []domain.ProbabilityAddressDTO{
...
func TestMapDomainToSerializable_LasFechasOpcionalesDePagoQuedanNilSiNoVienen(t *testing.T) {
	orden := ordenMinima()
	orden.Payments = []domain.ProbabilityPaymentDTO{{Amount: 50000, Status: "paid"}}
...
func TestMapDomainToSerializable_FormateaLasFechasDePagoQueSiVienen(t *testing.T) {
	orden := ordenMinima()
	orden.Payments = []domain.ProbabilityPaymentDTO{{
...
func TestMapDomainToSerializable_TrasladaLosEnviosConSusFechasOpcionales(t *testing.T) {
	orden := ordenMinima()
	orden.Shipments = []domain.ProbabilityShipmentDTO{{
...
func TestMapDomainToSerializable_MapeaElChannelMetadataCuandoViene(t *testing.T) {
	orden := ordenMinima()
	orden.ChannelMetadata = &domain.ProbabilityChannelMetadataDTO{
		ChannelSource: "canal",
		RawData:       []byte(`{"id":123}`),
		ReceivedAt:    momento(),
		IsLatest:      true,
...
func TestMapDomainToSerializable_ElResultadoEsSerializableSinPerderLaOrden(t *testing.T) {
	...
	orden.Currency = "COP"
	orden.OrderItems = []domain.ProbabilityOrderItemDTO{{ProductSKU: "SKU-1", Quantity: 1}}
