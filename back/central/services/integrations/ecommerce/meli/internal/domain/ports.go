...
type IMeliClient interface {
	...
	ForAccount(key string) IMeliClient
	...
	GetShipmentDetail(ctx context.Context, accessToken string, shipmentID int64) (*MeliShippingDetail, []byte, error)
	...
	GetItemsStock(ctx context.Context, accessToken string, itemIDs []string) ([]ChannelStock, error)
