...
func (r *MeliShippingDetailResponse) ToDomain() domain.MeliShippingDetail {
	...
	if r.Logistic != nil {
		if detail.LogisticType == "" {
			detail.LogisticType = r.Logistic.Type
		}
		if detail.LogisticMode == "" {
			detail.LogisticMode = r.Logistic.Mode
		}
	}
	...
	if r.ReceiverAddress != nil {
		ra := &domain.MeliReceiverAddress{
			...
			ReceiverName:  r.ReceiverAddress.ReceiverName,
			ReceiverPhone: r.ReceiverAddress.ReceiverPhone,
		}
	...
	if r.ShippingOption != nil {
		...
		if r.ShippingOption.EstimatedDeliveryTime != nil {
			opt.EstimatedDeliveryTime = &domain.MeliEstimatedDelivery{
