...
func NewInventoryPushConsumer(queue rabbitmq.IQueue, useCase usecases.IMeliUseCase, logger log.ILogger) *InventoryPushConsumer {
	return &InventoryPushConsumer{
		queue:   queue,
		useCase: useCase,
		logger:  logger.WithModule("meli.inventory_push_consumer"),
	}
}
...
func (c *InventoryPushConsumer) Start(ctx context.Context) {
	...
	if err := c.queue.DeclareQueue(rabbitmq.QueueMeliInventoryStockPush, true); err != nil {
		c.logger.Error(ctx).Err(err).Msg("Error al declarar la cola de push de stock MercadoLibre")
		return
	}

	go func() {
		err := c.queue.Consume(ctx, rabbitmq.QueueMeliInventoryStockPush, func(body []byte) error {
			c.handle(ctx, body)
			return nil
		})
		if err != nil {
			c.logger.Error(ctx).Err(err).Msg("Error al consumir la cola de push de stock MercadoLibre")
		}
	}()

	c.logger.Info(ctx).Msg("Consumer de push de stock MercadoLibre iniciado")
}

func (c *InventoryPushConsumer) handle(ctx context.Context, body []byte) {
	var msg ecommerceStockPushMessage
	if err := json.Unmarshal(body, &msg); err != nil {
		c.logger.Error(ctx).Err(err).Msg("Mensaje de push de stock MercadoLibre invalido")
		return
	}
	...
	if err := c.useCase.UpdateItemStock(ctx, integrationID, msg.ExternalProductID, msg.Quantity); err != nil {
		c.logger.Error(ctx).
			...
			Msg("Error al empujar stock a MercadoLibre")
	}
}
