...
const (
	channelLabel        = "Jumpseller"
	reconcileEventType  = "jumpseller.product.reconcile.completed"
...
func (uc *jumpsellerUseCase) ReconcileProductsAsync(ctx context.Context, integrationID string, businessID, integIDUint uint, correlationID string) {
	uc.emitSyncEvent(ctx, businessID, integIDUint, "jumpseller.product.reconcile.started", map[string]interface{}{"correlation_id": correlationID})

	result, err := uc.ReconcileProducts(ctx, integrationID, businessID)
	if err != nil {
		uc.logger.Error(ctx).Err(err).Str("integration_id", integrationID).Msg("Error al comparar el catalogo con Jumpseller")
...
func reconcileDetail(result *domain.ReconcileResult) []reconcileDetailItem {
	...
	add(result.OnlyInJumpseller, "solo en "+channelLabel, "warn", "only_channel")

	return detail
}

func (uc *jumpsellerUseCase) publishReconcileRun(ctx context.Context, businessID, integrationID uint, data map[string]interface{}) {
	if uc.rabbit == nil {
		return
	}
