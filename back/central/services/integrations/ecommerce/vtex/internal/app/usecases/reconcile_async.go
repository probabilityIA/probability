...
const (
	channelLabel       = "VTEX"
	reconcileEventType = "vtex.product.reconcile.completed"
)
...
func (uc *vtexUseCase) ReconcileProductsAsync(ctx context.Context, integrationID string, businessID, integIDUint uint, correlationID string) {
	uc.emitSyncEvent(ctx, businessID, integIDUint, "vtex.product.reconcile.started", map[string]interface{}{"correlation_id": correlationID})

	result, err := uc.ReconcileProducts(ctx, integrationID, businessID)
	if err != nil {
		uc.logger.Error(ctx).Err(err).Str("integration_id", integrationID).Msg("Error al comparar el catalogo con VTEX")
	...
	counts := map[string]interface{}{
		...
		"only_in_channel":     len(result.OnlyInVTEX),
...
func reconcileDetail(result *domain.ReconcileResult) []reconcileDetailItem {
	...
	add(result.OnlyInVTEX, "solo en "+channelLabel, "warn", "only_channel")

	return detail
}

func (uc *vtexUseCase) publishReconcileRun(ctx context.Context, businessID, integrationID uint, data map[string]interface{}) {
	if uc.rabbit == nil {
		return
	}
