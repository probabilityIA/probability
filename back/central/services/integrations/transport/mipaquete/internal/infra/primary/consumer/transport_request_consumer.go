...
import (
	...
	"github.com/secamc93/probability/back/central/services/integrations/transport/mipaquete/internal/app"
	"github.com/secamc93/probability/back/central/services/integrations/transport/mipaquete/internal/infra/secondary/queue"
...
func NewTransportRequestConsumer(
	...
	return &TransportRequestConsumer{
		...
		log:                logger.WithModule("transport.mipaquete.consumer"),
	}
}

// Start starts consuming from the MiPaquete requests queue
func (c *TransportRequestConsumer) Start(ctx context.Context) error {
	...
	c.log.Info(ctx).
		Str("queue", QueueMiPaqueteRequests).
		Msg("Starting MiPaquete transport request consumer")

	if err := c.rabbit.DeclareQueue(QueueMiPaqueteRequests, true); err != nil {
		c.log.Error(ctx).Err(err).Msg("Failed to declare queue")
		return err
	}

	if err := c.rabbit.Consume(ctx, QueueMiPaqueteRequests, c.handleRequest); err != nil {
...
func (c *TransportRequestConsumer) resolveAPIKey(ctx context.Context, request *TransportRequestMessage) (string, error) {
	if request.IntegrationID == 0 {
		if envKey := os.Getenv("MIPAQUETE_API_KEY"); envKey != "" {
			c.log.Warn(ctx).Msg("integration_id is 0, using MIPAQUETE_API_KEY env var fallback")
			return envKey, nil
		}
	...
	if err != nil {
		if envKey := os.Getenv("MIPAQUETE_API_KEY"); envKey != "" {
			c.log.Warn(ctx).
				Uint("integration_id", request.IntegrationID).
				Msg("Failed to decrypt credentials, using MIPAQUETE_API_KEY env var fallback")
			return envKey, nil
		}
...
// TODO: Implement when MiPaquete API is ready
func (c *TransportRequestConsumer) processQuote(ctx context.Context, request *TransportRequestMessage, apiKey string) *queue.TransportResponseMessage {
	c.log.Warn(ctx).Msg("⚠️ MiPaquete processQuote not yet implemented")
	return c.errorResponse(request, "mipaquete: quote not yet implemented")
}
...
func (c *TransportRequestConsumer) processGenerate(ctx context.Context, request *TransportRequestMessage, apiKey string) *queue.TransportResponseMessage {
	c.log.Warn(ctx).Msg("⚠️ MiPaquete processGenerate not yet implemented")
	return c.errorResponse(request, "mipaquete: generate not yet implemented")
}
...
func (c *TransportRequestConsumer) processTrack(ctx context.Context, request *TransportRequestMessage, apiKey string) *queue.TransportResponseMessage {
	c.log.Warn(ctx).Msg("⚠️ MiPaquete processTrack not yet implemented")
	return c.errorResponse(request, "mipaquete: track not yet implemented")
}
...
func (c *TransportRequestConsumer) processCancel(ctx context.Context, request *TransportRequestMessage, apiKey string) *queue.TransportResponseMessage {
	c.log.Warn(ctx).Msg("⚠️ MiPaquete processCancel not yet implemented")
	return c.errorResponse(request, "mipaquete: cancel not yet implemented")
}
